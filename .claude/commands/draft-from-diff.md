---
description: Draft commit message from current git diff via local-mcp-toolbelt bridge
allowed-tools:
  - Bash
  - mcp__local-mcp__diff-semantic-index
---
...
3. Call `mcp__local-mcp__diff-semantic-index` with `source_uri=file:///tmp/omcp-diff-$TIMESTAMP.txt`. Do NOT pass the diff inline as `diff_text` — large diffs exceed ARG_MAX on macOS.
